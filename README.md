...
To establish a baseline, we trained EnlightenGAN model using the original set of hyperparameters while capturing the loss on a visdom server. The loss function in GAN considers two distinct contributions - the distribution generated by the GAN and the distribution of the real data as demonstrated by the visdom server's loss over time graph below.

![](./assets/visdom/visdom_final.png)

To assess the quality of their model, the authors of the original EnlightenGAN paper rely, instead, on both quantitative and qualitative measures of model performance. We adopted a similar approach conducting a survey for human subjective evaluation but adopted a different quantitative measure. While several measures have been introduced for evaluating and comparing GANs, as of yet, there is no consensus as to which measure best captures strengths and limitations of models and should be used for fair model comparison (see [[2]](https://arxiv.org/abs/1802.03446v5)). Whereas the authors of the original paper use Natural Image Quality Evaluator (NIQE) score, we decided to use Fréchet Inception Distance (FID).   
FID is known to perform well in terms of discriminability, robustness and computational efficiency. Therefore, we selected this alternative metric as a means to validate or invalidate the authors' findings.
...
where *Tr* sums up all the diagonal element.
...
* [1] Yifan Jiang, Xinyu Gong, Ding Liu, Yu Cheng, Chen Fang, Xiaohui Shen, Jianchao Yang, Pan Zhou and Zhangyang Wang. **EnlightenGAN: Deep Light Enhancement without Paired Supervision.** 2019. https://arxiv.org/abs/1906.06972  
